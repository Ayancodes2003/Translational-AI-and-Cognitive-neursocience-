...
def create_dataloaders(data_path, modality, batch_size=32):
    ...
    if modality in ['eeg', 'audio', 'text']:
        # Load dataset
        dataset_path = os.path.join(data_path, f'{modality}', 'processed', f'{modality}_dataset.pkl')
        ...
        dataset_path = os.path.join(data_path, 'fusion', 'processed', f'fusion_{fusion_type}_dataset.pkl')
        logger.info(f"Loading fusion dataset from {dataset_path}")
        with open(dataset_path, 'rb') as f:
            dataset = pickle.load(f)
